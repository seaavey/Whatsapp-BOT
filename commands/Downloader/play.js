...
export const run = async (m, { sock }) => {
  try {
    if (!m.text) return m.reply("Berikan Nama Lagu atau Nama Musik")
    ...
    if (!res.status) return m.reply("Maaf, lagu tidak ditemukan.")
    const data = res.data[0]
    if (convertDurationToSeconds(data.duration) > 600) return m.reply("Maaf, lagu terlalu lama.")
    await m.react("⏳")
    await m.reply("Sedang memproses...")
    const audio = await spotifydl(data.track_url)
    if (!audio || !audio.url) return m.reply("Maaf, lagu tidak ditemukan.")
