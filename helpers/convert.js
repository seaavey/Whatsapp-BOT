...
export const temp = process.platform === "win32" ? process.env.TEMP : "./temp";
    ...
    try {
        await new Promise((resolve, reject) => {
            ff(tmpFileIn)
                ...
                .on("end", () => resolve(true))
                //.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale=512:512:force_original_aspect_ratio=increase,fps=15,crop=512:512`]).toFormat('webp').save(tmpFileOut)
                .addOutputOptions(["-vcodec", "libwebp", "-vf", "scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=000000 [p]; [b][p] paletteuse", "-loop", "0", "-ss", "00:00:00", "-t", "00:00:05", "-preset", "default", "-an", "-vsync", "0"])
                .toFormat("webp")
                .saveToFile(tmpFileOut);
        });
        fs.promises.unlink(tmpFileIn);
...
export async function writeExif(media, metadata) {
    ...
    if (metadata && Object?.keys(metadata).length !== 0) {
        ...
        const json = {
            "sticker-pack-id": metadata?.packId || `Seaavey-${Date.now()}`,
            "sticker-pack-name": metadata?.packName || "WhatsApp Sticker By",
            "sticker-pack-publisher": metadata?.packPublish || "Seaavey-BOT",
